...
- [scan_bank_links.py](file://tools/scan_bank_links.py)
- [rename_battle_to_war.py](file://tools/rename_battle_to_war.py)
- [bank_switch_map.md](file://code/bank_switch_map.md)
- [bank_switch_scan_raw.txt](file://code/bank_switch_scan_raw.txt)
...
## Update Summary
**Changes Made**
- Updated terminology throughout ROM analysis documentation to reflect standardized battle/war system references
- Modified bank characterization sections to use consistent war/battle terminology
- Updated analysis methodology descriptions to align with current codebase terminology
- Enhanced bank switching architecture documentation with updated function names and labels
- Revised inter-bank communication patterns to reflect renamed procedures and data structures
...
8. [Bank Switching Architecture](#bank-switching-architecture)
9. [Inter-Bank Communication Patterns](#inter-bank-communication-patterns)
...
- **Updated** Advanced ROM verification tools for systematic byte-level validation and pattern analysis with standardized terminology
- **Updated** Comprehensive bank validation and address checking capabilities for ROM-accurate disassembly using consistent war/battle terminology
- **Updated** Complete bank switching architecture documentation with inter-bank communication patterns reflecting standardized terminology
- **Updated** Systematic linkage maps showing all cross-bank call sites and data access patterns with updated function names
...
- **Updated** Advanced verification pipeline for ROM-accurate validation and debugging with standardized terminology
- **Updated** Bank switching architecture analysis with comprehensive linkage mapping using updated function names
...
AE --> AF["scan_bank_links.py<br/>Bank switching analysis"]
AF --> AG["bank_switch_map.md<br/>Linkage documentation"]
```
...
- [transform_wrap.py:134-303](file://transform_wrap.py#L134-303)
...
- [scan_bank_links.py:1-329](file://tools/scan_bank_links.py#L1-L329)
...
- **Updated** Bank analyzers: Specialized analysis of bank 0x1F for vectors, reset handler, bank switching, and key functions with comprehensive function boundary detection using standardized terminology
- Disassemblers: Basic 6502 disassembler and a comprehensive bank 0x1F disassembler with labeled regions
- **Updated** Advanced verification tools: Systematic ROM verification including disassembly validation, range checking, pattern searching, and byte-level accuracy with consistent terminology
- **Updated** Bank validation tools: Address checking, bank-specific verification, and CHR table dumping for ROM-accurate analysis with updated function names
- **Updated** Pattern search tools: Targeted searches for specific opcodes and memory patterns across the ROM with standardized terminology
- **Updated** Bank switching analysis tool: Comprehensive scanning of bank switching call sites and generation of linkage maps with updated function references
...
**Updated** The enhanced analysis pipeline now includes automated validation, comprehensive verification tools, systematic ROM-accurate analysis capabilities, and complete bank switching architecture documentation with inter-bank communication patterns using standardized terminology.
...
subgraph "Bank Switching Architecture"
SWITCH["Bank Switch Primitives"] --> SLOT_A["Slot A Context<br/>$DE/$DF/$E0"]
SWITCH --> SLOT_B["Slot B Context<br/>$E1/$E2/$E3"]
SWITCH --> TRAMP["Trampoline System"]
TRAMP --> CALLBACK["Callback Mechanism"]
SLOT_A --> NMI_SAFE["NMI-Safe Context Switching"]
SLOT_B --> NMI_SAFE
end
```
...
- [dump_correct_bytes.py:1-35](file://tools/dump_correct_bytes.py#L1-L35)
- [search_0530.py:1-23](file://tools/search_0530.py#L1-L23)
- [bank_switch_map.md:14-38](file://code/bank_switch_map.md#L14-L38)
...
**Updated** The enhanced bank $1f analysis now includes comprehensive function boundary detection and internal JSR target analysis with standardized terminology.
...
- **Updated** The specialized analyzers now provide comprehensive function boundary detection and internal JSR target analysis with standardized terminology:
...
- **Updated** Example interpretation with new capabilities:
  ...
  - **Updated** Function boundary detection reveals 45+ functions in bank 0x1F with internal call patterns using standardized terminology
  - **Updated** Gap detection identifies 15+ data gaps requiring ROM-accurate byte insertion
  - **Updated** Bank switching analysis shows 75+ call sites in bank 0x1F with comprehensive linkage mapping using updated function names
- Guidance:
  ...
  - **Updated** Leverage transformation pipeline for automated assembly validation and gap detection
  - **Updated** Utilize advanced verification tools for systematic ROM-accurate validation
  - **Updated** Analyze bank switching patterns to understand inter-bank communication flows with standardized terminology
...
K --> M
L --> M
M --> N["scan_bank_links.py<br/>Bank switching analysis"]
N --> O["bank_switch_map.md<br/>Linkage documentation"]
```
...
### Switching Primitives and Context Management
The bank switching system uses two independent slots (A and B) for NMI-safe context switching:

| Primitive | Address | Windows Affected | Shadow RAM | Purpose |
|-----------|---------|------------------|------------|---------|
| `SwitchBankAC_B` | $F237 | $A000 = Y, $C000 = Y+1 | $00E2 / $00E3 (slot B) | Set A/C windows using slot B context |
| `SwitchBankAC_A` | $F24B | $A000 = Y, $C000 = Y+1 | $00DF / $00E0 (slot A) | Set A/C windows using slot A context |
| `SwitchBank8_B` | $F25F | $8000 = Y | $00E1 (slot B) | Set 8KB window using slot B context |
| `SwitchBank8_A` | $F266 | $8000 = Y | $00DE (slot A) | Set 8KB window using slot A context |
| `BankedCallbackTrampoline` | $EE07 | $A000/$C000 via slot B | Saves caller bank to $0058 | Trampoline for banked callbacks |
| `BankedCallbackReturn` | $EE4D | Restores slot B from stack | — | Return from banked callback |
| `BankSwitch` | $E51F | **CHR only** | $00E6-$00ED | Configure CHR pages (no PRG effect) |

**Key Features:**
- Effective PRG bank = `Y & $1F` (32 x 8KB banks)
- Callers use `Y = $2x` (slot-A context) or `Y = $3x` (slot-B context)
- `$A000` writes additionally set bits 6-7 (`ORA #$C0`, CHR-RAM disable)
- Two independent slots enable NMI-safe context switching

**Section sources**
- [bank_switch_map.md:14-38](file://code/bank_switch_map.md#L14-L38)

### Inter-Bank Communication Patterns

#### Linkage Matrix Overview
The bank switching creates a complex web of inter-bank communications:

| Source Bank | Primary Role | $A000/$C000 Targets | $8000 Targets |
|-------------|--------------|---------------------|---------------|
| **$1F** (boot/NMI) | State machine, NMI handlers | $08+$09, $0A+$0B, $0C+$0D, $0E+$0F, $17+$18, $19+$1A, $1B+$1C, $1D+$1E | $02 (sound), $10, $11, $12, $15, $16 |
| **$08+$09** (war/AI) | War engine, AI logic | $0E (trampoline), $1B+$1C, $1D+$1E, $19 (trampoline) | $00, $03, $04, $05 (terrain), $06, $10, $11 |
| **$0A+$0B** (AI/domestic) | AI turn processing, domestic affairs | $17+$18 (trampoline) | $01 (sprites), $10, $11 |
| **$0C+$0D** (officer/march) | Officer commands, march logic | $00, $08+$09, $0A+$0B, $0E, $17+$18, $19, $1D+$1E | $06, $10, $11 |
| **$17+$18** (display) | Display engine, domestic display | $19 (trampoline), $1D+$1E (trampoline) | $00, $03, $04, $05 (war scene), $01, $06, $10, $11, $14, $15 |
| **$1D+$1E** (menus/SRAM) | Menu system, SRAM management | $1D+$1E (self trampoline) | $01, $10, $11, $12, $13, $16 |

#### Trampoline Calling Convention
The trampoline system enables safe cross-bank calls:
```assembly
LDY #target_bank
JSR BankedCallbackTrampoline
.word target_address
```

The target routine must end with:
```assembly
JSR BankedCallbackReturn  ; or JMP BankedCallbackReturn
```

**Section sources**
- [bank_switch_map.md:42-55](file://code/bank_switch_map.md#L42-L55)
- [bank_switch_map.md:26-38](file://code/bank_switch_map.md#L26-L38)

### Comprehensive Linkage Analysis

#### Per-Bank Communication Details

**Bank $1F (Boot/NMI):**
- 75 bank switching call sites detected
- Extensive state machine with 15 vector entries
- NMI sub-dispatch handling multiple display tasks
- Heavy usage of both $A000/$C000 pair switches and $8000 data access

**Bank $08+$09 (War/AI):**
- Dynamic terrain bank selection via TileBankTable
- Multiple trampoline calls to other banks
- Heavy data access to banks $10, $11, $06

**Bank $0C+$0D (Officer/March):**
- All outbound $A000/$C000 traffic uses trampolines
- Complex officer command and march logic
- Extensive interaction with war and display systems

**Section sources**
- [bank_switch_scan_raw.txt:1-200](file://code/bank_switch_scan_raw.txt#L1-L200)
- [bank_switch_map.md:59-235](file://code/bank_switch_map.md#L59-L235)

## Inter-Bank Communication Patterns

### Call Site Analysis
The scan_bank_links.py tool provides comprehensive analysis of bank switching patterns:

#### Detection Capabilities:
1. Direct bank switching calls (JSR SwitchBankAC_A/B, SwitchBank8_A/B)
2. Trampoline calls with inline target addresses
3. NMI-style direct jumps after bank switching
4. CHR configuration switches (BankSwitch)
5. Dynamic bank selection patterns

#### Analysis Output:
- Per-file call site tables with exact addresses and line numbers
- Aggregated linkage maps showing source-to-target relationships
- Symbol resolution using functions.h for better readability
- Pattern detection for common bank switching idioms
...
The game uses a sophisticated state machine in bank $1F that coordinates activities across multiple banks:

1. **State Initialization**: Sets up appropriate banks for each game state
2. **NMI Coordination**: Handles display updates while preserving execution context
3. **Cross-Bank Calls**: Uses trampolines for safe inter-bank communication
4. **Data Access**: Efficiently switches between code and data banks

#### Memory Management:
- **Slot-Based Context**: Independent A/B slots prevent NMI conflicts
- **Dynamic Banking**: Runtime bank selection based on game state
- **Data Prefetching**: Strategic bank switching for optimal performance

**Section sources**
- [bank_switch_map.md:61-121](file://code/bank_switch_map.md#L61-L121)
- [bank_switch_map.md:238-261](file://code/bank_switch_map.md#L238-L261)
...
- **Updated** transform_* tools provide automated assembly validation and gap detection with standardized terminology
- **Updated** verify_* tools provide systematic ROM-accurate validation and debugging with updated function names
- **Updated** check_* and search_* tools provide specialized verification and pattern analysis with consistent terminology
- **Updated** scan_bank_links.py analyzes bank switching patterns and generates linkage documentation with updated references
...
Verify --> BankLinks["scan_bank_links.py"]
BankLinks --> LinkMap["bank_switch_map.md"]
...
- **Updated** Transformation pipeline adds computational overhead but improves accuracy:
...
- **Updated** Advanced verification tools add significant computational overhead for comprehensive ROM validation:
...
- **Updated** Bank switching analysis adds additional processing overhead:
  - Scans multiple disassembled files for bank switching patterns
  - Resolves symbols and builds comprehensive linkage maps
  - Processes trampoline patterns and callback mechanisms with updated function names
...
- Recommendations:
  ...
  - **Updated** Utilize enhanced transformation pipeline for automated validation and gap detection
  - **Updated** Employ systematic verification approach for ROM-accurate results
  - **Updated** Analyze bank switching patterns to optimize disassembly strategy with standardized terminology
...
- **Updated** Bank switching analysis issues:
  - Missing symbol resolutions: ensure functions.h is up-to-date with current bank addresses and updated function names
  - Incomplete linkage maps: verify all disassembled bank files are present and properly formatted
  - Trampoline detection failures: check for consistent calling conventions across banks
  - Slot context confusion: verify proper A/B slot usage in bank switching calls
- **Updated** Linkage documentation problems:
  - Inconsistent bank numbering: ensure consistent hex notation throughout analysis
  - Missing cross-references: verify symbol names match between banks with updated terminology
  - Dynamic bank selection: account for runtime bank switching patterns
- Build verification fails:
  - Confirm rom_info.h and bank stubs are consistent with the ROM layout
  - **Updated** Check transformation pipeline output for validation errors
  - **Updated** Verify advanced verification tools produce expected results
  - **Updated** Validate bank switching patterns against actual ROM behavior with standardized terminology
...
- [search_chr_loader.py:1-100](file://tools/search_chr_loader.py#L1-L100)
- [search_chr_loader2.py:1-100](file://tools/search_chr_loader2.py#L1-L100)
- [scan_bank_links.py:241-329](file://tools/scan_bank_links.py#L241-L329)
...
- **Updated** Leveraging the transformation pipeline for automated assembly validation, gap detection, and address alignment verification with standardized terminology
- **Updated** Utilizing advanced verification tools for systematic ROM-accurate validation and debugging with updated function names
- **Updated** Employing specialized tools for pattern searching, bank validation, and CHR table verification with consistent terminology
- **Updated** Understanding the complete bank switching architecture with slot-based context management using updated function references
- **Updated** Analyzing comprehensive linkage maps showing all inter-bank communication patterns with standardized terminology
- **Updated** Utilizing the scan_bank_links.py tool for systematic bank switching analysis with updated function names
- Using the enhanced analysis tools to plan disassembly, annotate assembly, and verify accuracy against the original ROM
- **Updated** Integrating comprehensive bank switching documentation to understand the game's execution flow across multiple banks with standardized terminology
...
- **Updated** Slot-based banking: Independent A/B contexts for NMI safety with updated function names
- **Updated** Trampoline system: Safe cross-bank calling mechanism with standardized terminology
...
- **Updated** Transformation pipeline ensures accurate interrupt vector handling and validation with standardized terminology
- **Updated** Advanced verification tools provide systematic ROM-accurate validation of interrupt handling with updated function names
- **Updated** Bank switching maintains execution context during interrupts using dual-slot architecture with consistent terminology

**Section sources**
- [PROJECT.md:101-117](file://PROJECT.md#L101-L117)
- [bank_1f_analysis.md:22-76](file://code/bank_1f_analysis.md#L22-L76)
- [bank_switch_map.md:36-38](file://code/bank_switch_map.md#L36-L38)
...
### Appendix E: Bank Switching Analysis Tools
- **scan_bank_links.py**: Comprehensive scanning of bank switching call sites and generation of linkage documentation
- **bank_switch_map.md**: Detailed documentation of bank switching architecture and inter-bank communication patterns with updated terminology
- **bank_switch_scan_raw.txt**: Raw output from bank switching analysis with per-call-site details
- **rename_battle_to_war.py**: Tool for standardizing battle/war terminology throughout the codebase

**Section sources**
- [scan_bank_links.py:1-329](file://tools/scan_bank_links.py#L1-L329)
- [bank_switch_map.md:1-307](file://code/bank_switch_map.md#L1-L307)
- [bank_switch_scan_raw.txt:1-300](file://code/bank_switch_scan_raw.txt#L1-L300)
- [rename_battle_to_war.py:1-135](file://tools/rename_battle_to_war.py#L1-L135)