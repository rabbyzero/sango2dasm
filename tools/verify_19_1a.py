#!/usr/bin/env python3
"""Standalone byte verification for asm/banks/prg_19_1a.asm.
...
rom/prg/prg_19.bin + rom/prg/prg_1a.bin.
"""
import re
import subprocess
import sys
...
src = open("asm/banks/prg_19_1a.asm").read()
# Fix the PC at the bank bases so relative branches resolve as in the real layout
src = src.replace('.segment "CODE_BANK19"',
                  '.segment "CODE_BANK19"\n.org $A000', 1)
src = src.replace('.segment "CODE_BANK1A"',
                  '.segment "CODE_BANK1A"\n.org $C000', 1)
...
cfg = """
MEMORY {
    BANK19: start = $A000, size = $2000, type = ro, fill = yes, fillval = $FF,
            file = "build/_bank1b.bin";
    BANK1A: start = $C000, size = $2000, type = ro, fill = yes, fillval = $FF,
            file = "build/_bank1c.bin";
}
SEGMENTS {
    CODE_BANK19: load = BANK19, type = ro;
    CODE_BANK1A: load = BANK1A, type = ro;
}
"""
