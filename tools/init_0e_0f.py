#!/usr/bin/env python3
"""
Initialize asm/banks/prg_0e_0f.asm from output/prg_0e_0f_raw.asm.
...
Steps:
  ...
  4. Regenerate code/data region markers and insert .segment directives:
     CODE_BANK0E for $A000-$BFFF, CODE_BANK0F for $C000-$DFFF.
...
RAW_PATH = "output/prg_0e_0f_raw.asm"
OUT_PATH = "asm/banks/prg_0e_0f.asm"
BANK_LO = "rom/prg/prg_0e.bin"
BANK_HI = "rom/prg/prg_0f.bin"
BOUNDARY = 0xC000
...
def main():
    ...
    def switch_seg(addr):
        nonlocal cur_seg
        seg = "CODE_BANK0E" if addr < BOUNDARY else "CODE_BANK0F"
        if seg != cur_seg:
            out_lines.append("")
