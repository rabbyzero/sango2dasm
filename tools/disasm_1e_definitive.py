...
Correctly handles tile data blocks between code routines.
"""
import sys
...
# 1. $FF padding ($DF6E-$DFFF)
for i in range(0x1F6E, SIZE):
    data_regions.add(i)

# 2. Pointer table at $DF1A-$DF6D
for i in range(0x1F1A, 0x1F6E):
    data_regions.add(i)

# 3. Lookup tables after JMP $C934 (not callback code)
for i in range(SIZE - 2):
    if data[i] == 0x4C and data[i+1] == 0x34 and data[i+2] == 0xC9:
        after = i + 3
        # Skip if callback code follows
        ...
        for j in range(after, min(after + 64, SIZE - 4)):
            if data[j] == 0xA9 and data[j+2] == 0x8D and data[j+3] == 0x10 and data[j+4] == 0x00:
                for k in range(after, j): data_regions.add(k)
                break
...
# 4. Tile data blocks after RTS in the $C9E8-$DE80 range
# Code-start bytes: opcodes that commonly begin a routine
CODE_START_BYTES = {
    0xA9,  # LDA #imm
    0xAD,  # LDA abs
    0xA5,  # LDA zp
...
# Find all RTS positions in the $C9E0-$DE80 range
for i in range(0x09E0, 0x1E80):
    if data[i] == 0x60:  # RTS
        after = i + 1
        if after >= SIZE:
            continue
        ...
        # If the byte after RTS is a common code-start opcode, it's code
        if next_byte in CODE_START_BYTES:
            continue
        
        # Otherwise it's a tile data block - find where it ends
        block_start = after
        block_end = after
        for j in range(after, min(after + 300, SIZE)):
            b = data[j]
            # Check if this position starts a code block
            if b in CODE_START_BYTES:
                # Verify it's really code by checking the next few bytes
                if b == 0xA9 and j + 1 < SIZE:  # LDA #imm
                    block_end = j
                    break
                if b == 0xAD and j + 2 < SIZE:  # LDA abs
                    block_end = j
                    break
                if b == 0x8D and j + 2 < SIZE:  # STA abs
                    block_end = j
                    break
                if b == 0x20 and j + 2 < SIZE:  # JSR
                    block_end = j
                    break
                if b == 0x4C:  # JMP
                    block_end = j
                ...
                if b == 0xAE and j + 2 < SIZE:  # LDX abs
                    block_end = j
                    break
                if b == 0xCE and j + 2 < SIZE:  # DEC abs
                    block_end = j
                    break
                if b == 0xEE and j + 2 < SIZE:  # INC abs
                    block_end = j
                    break
                if b == 0xA5 and j + 1 < SIZE:  # LDA zp
                    block_end = j
                    break
                if b == 0x85 and j + 1 < SIZE:  # STA zp
                    block_end = j
                    break
                if b == 0x48:  # PHA
                    block_end = j
                ...
                if b == 0x68:  # PLA
                    block_end = j
                    break
                if b == 0x18 or b == 0x38 or b == 0x78 or b == 0x58:  # CLC/SEC/SEI/CLI
                    block_end = j
                    break
                if b == 0xA8 or b == 0xAA or b == 0x98 or b == 0x8A:  # TAY/TAX/TYA/TXA
                    block_end = j
                    break
                if b == 0x84 and j + 1 < SIZE:  # STY zp
                    block_end = j
                    break
                if b == 0x8C and j + 2 < SIZE:  # STY abs
                    block_end = j
                    break
                if b == 0x86 and j + 1 < SIZE:  # STX zp
                    block_end = j
                    break
                if b == 0x8E and j + 2 < SIZE:  # STX abs
                    block_end = j
                    break
                if b == 0x00:  # BRK
                    block_end = j
                ...
                if b == 0xEA:  # NOP
                    block_end = j
                    break
                if b == 0xE8 or b == 0xCA or b == 0xC8 or b == 0x88:  # INX/DEX/INY/DEY
                    block_end = j
                    break
            block_end = j + 1
...
for (off, sz, is_code) in instrs:
    ...
    if mode == "rel":
        ...
        target = addr + 2 + delta
        if BASE <= target < BASE + SIZE: set_label(target)
...
def fmt_op(off, sz, mn, mode):
    ...
    if mode == "abs":
        ...
        return f"${target:04X}"
    if mode == "abx": return f"${data[off+1] | (data[off+2] << 8):04X},X"
    if mode == "aby": return f"${data[off+1] | (data[off+2] << 8):04X},Y"
    if mode == "ind": return f"(${data[off+1] | (data[off+2] << 8):04X})"
...
while pos_idx < len(instrs):
    ...
    if off in proc_map:
        if in_proc: out.append(".endproc"); out.append("")
        name = proc_map[off]
        out.append(f";===============================================================================")
...
if in_proc: out.append(".endproc")

# Verify byte count (skip comment-only and directive-only lines)
total = 0
for line in out:
    s = line.strip()
    if s.startswith(';') or s.startswith('.proc') or s.startswith('.endproc') or s.startswith('.segment') or s == '': continue
    m = line.find('; $')
    if m >= 0:
        rest = line[m+3:]
        c = rest.find(':')
        if c >= 0:
            total += len(rest[c+1:].strip().split())

print(f"; Total bytes: {total} / {SIZE}", file=sys.stderr)
if total != SIZE: print(f"; WARNING: off by {SIZE - total}", file=sys.stderr)
print("\n".join(out))
