#!/usr/bin/env python3
"""
Initialize asm/banks/prg_19_1a.asm from output/prg_19_1a_raw.asm.
...
Steps:
  ...
  4. Regenerate code/data region markers and insert .segment directives:
     CODE_BANK19 for $A000-$BFFF, CODE_BANK1A for $C000-$DFFF.
...
RAW_PATH = "output/prg_19_1a_raw.asm"
OUT_PATH = "asm/banks/prg_19_1a.asm"
BANK_LO = "rom/prg/prg_19.bin"
BANK_HI = "rom/prg/prg_1a.bin"
BOUNDARY = 0xC000
...
def is_bad(instr):
    ...
    if mn == "NOP" and instr["operand"]:
        return True
    if mn == "NOP" and not instr["operand"] and instr["bytes"][0] != 0xEA:
        # illegal 1-byte NOP variants ($1A/$3A/$5A/$7A/$DA/$FA);
        # ca65 would emit $EA instead
        return True
...
def main():
    ...
    def switch_seg(addr):
        nonlocal cur_seg
        seg = "CODE_BANK19" if addr < BOUNDARY else "CODE_BANK1A"
        if seg != cur_seg:
            out_lines.append("")
