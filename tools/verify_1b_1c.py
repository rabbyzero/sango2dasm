#!/usr/bin/env python3
"""Standalone byte verification for asm/banks/prg_1b_1c.asm.
...
rom/prg/prg_1b.bin + rom/prg/prg_1c.bin.
"""
import re
import subprocess
import sys
...
src = open("asm/banks/prg_1b_1c.asm").read()
# Fix the PC at the bank bases so relative branches resolve as in the real layout
src = src.replace('.segment "CODE_BANK1B"',
                  '.segment "CODE_BANK1B"\n.org $A000', 1)
src = src.replace('.segment "CODE_BANK1C"',
                  '.segment "CODE_BANK1C"\n.org $C000', 1)
...
cfg = """
MEMORY {
    BANK1B: start = $A000, size = $2000, type = ro, fill = yes, fillval = $FF,
            file = "build/_bank1b.bin";
    BANK1C: start = $C000, size = $2000, type = ro, fill = yes, fillval = $FF,
            file = "build/_bank1c.bin";
}
SEGMENTS {
    CODE_BANK1B: load = BANK1B, type = ro;
    CODE_BANK1C: load = BANK1C, type = ro;
}
"""
...
assembled = (open("build/_bank1b.bin", "rb").read() +
             open("build/_bank1c.bin", "rb").read())
rom = (open("rom/prg/prg_1b.bin", "rb").read() +
       open("rom/prg/prg_1c.bin", "rb").read())
